...
class TestBranchForking:
    ...
    def test_fork_with_system_message(self):
        """Test forking preserves system message reference with system=True."""
        session = Session()
        ...
        # Verify system message is SHARED (same reference, not cloned)
        original_system = session.get_branch_system(original)
        cloned_system = session.get_branch_system(cloned)
        assert original_system.id == cloned_system.id  # Same message (shared reference)
